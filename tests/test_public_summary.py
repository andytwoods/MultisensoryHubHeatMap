...
def test_summary_requires_auth(client):
    resp = client.get(reverse("concept_analytics_summary"))
    assert resp.status_code == 401

@pytest.mark.django_db
def test_summary_wrong_token(client):
    resp = client.get(reverse("concept_analytics_summary"), HTTP_AUTHORIZATION="Bearer wrong")
    assert resp.status_code == 401
...
def test_summary_returns_correct_structure(client):
    ...
    resp = client.get(reverse("concept_analytics_summary"), HTTP_AUTHORIZATION=AUTH)
...
def test_summary_post_not_allowed(client):
    resp = client.post(reverse("concept_analytics_summary"), HTTP_AUTHORIZATION=AUTH)
